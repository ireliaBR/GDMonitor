...
//#import "NSObject+GD_Reflection.h"
//#import "GDTypeEncodingParser.h"
//#import "GDMethod.h"

NSString * const GDRuntimeUtilityErrorDomain = @"GDRuntimeUtilityErrorDomain";

@implementation GDRuntimeUtility

#pragma mark - General Helpers (Public)

+ (BOOL)pointerIsValidObjcObject:(const void *)pointer {
    return GDPointerIsValidObjcObject(pointer);
}
...
+ (id)potentiallyUnwrapBoxedPointer:(id)returnedObjectOrNil type:(const GDTypeEncoding *)returnType {
    ...
    NSInteger i = 0;
    if (returnType[i] == GDTypeEncodingConst) {
        i++;
    }

    BOOL returnsObjectOrClass = returnType[i] == GDTypeEncodingObjcObject ||
                                returnType[i] == GDTypeEncodingObjcClass;
    BOOL returnsVoidPointer   = returnType[i] == GDTypeEncodingPointer &&
                                returnType[i+1] == GDTypeEncodingVoid;
    BOOL returnsCString       = returnType[i] == GDTypeEncodingCString;
    ...
    if (!returnsObjectOrClass) {
        ...
        if (returnsCString) {
            ...
            if ([GDRuntimeUtility pointerIsValidObjcObject:value.pointerValue]) {
...
+ (NSUInteger)fieldNameOffsetForTypeEncoding:(const GDTypeEncoding *)typeEncoding {
    ...
    while (typeEncoding[beginIndex] == GDTypeEncodingQuote) {
        NSUInteger endIndex = beginIndex + 1;
        while (typeEncoding[endIndex] != GDTypeEncodingQuote) {
            ++endIndex;
        }
...
//    NSArray<Class> *classes = GDGetAllSubclasses(NSClassFromString(className), NO);
//    NSArray<GDObjectRef *> *references = [GDObjectRef referencingClasses:classes];
//    return references;
//}
...
+ (NSArray<NSString *> *)allPropertyAttributeKeys {
    ...
    dispatch_once(&onceToken, ^{
        allPropertyAttributeKeys = @[
            kGDPropertyAttributeKeyTypeEncoding,
            kGDPropertyAttributeKeyBackingIvarName,
            kGDPropertyAttributeKeyReadOnly,
            kGDPropertyAttributeKeyCopy,
            kGDPropertyAttributeKeyRetain,
            kGDPropertyAttributeKeyNonAtomic,
            kGDPropertyAttributeKeyCustomGetter,
            kGDPropertyAttributeKeyCustomSetter,
            kGDPropertyAttributeKeyDynamic,
            kGDPropertyAttributeKeyWeak,
            kGDPropertyAttributeKeyGarbageCollectable,
            kGDPropertyAttributeKeyOldStyleTypeEncoding,
        ];
    });
...
+ (NSArray<NSString *> *)prettyArgumentComponentsForMethod:(Method)method {
    ...
    for (unsigned int argIndex = 0; argIndex < selectorComponents.count; argIndex++) {
        char *argType = method_copyArgumentType(method, argIndex + kGDNumberOfImplicitArgs);
...
//            *error = [NSError
//                errorWithDomain:GDRuntimeUtilityErrorDomain
//                code:GDRuntimeUtilityErrorCodeDoesNotRecognizeSelector
//                userInfo:userInfo
//            ];
//        }
//
...
//    NSMethodSignature *methodSignature = [GDMethod selector:selector class:cls].signature;
//    if (!methodSignature) {
...
//        methodSignature.methodReturnType[0] != GDTypeEncodingVoid) {
//        return nil;
//    }
//
...
//    for (NSUInteger argumentIndex = kGDNumberOfImplicitArgs; argumentIndex < numberOfArguments; argumentIndex++) {
//        NSUInteger argumentsArrayIndex = argumentIndex - kGDNumberOfImplicitArgs;
...
//            if (typeEncodingCString[0] == GDTypeEncodingObjcObject ||
//              typeEncodingCString[0] == GDTypeEncodingObjcClass ||
...
//                        *error = [NSError
//                            errorWithDomain:GDRuntimeUtilityErrorDomain
//                            code:GDRuntimeUtilityErrorCodeArgumentTypeMismatch
//                            userInfo:userInfo
//                        ];
//                    }
//                    return nil;
...
//                    GDGetSizeAndAlignment(typeEncodingCString, &bufferSize, NULL);
//
//                    if (bufferSize > 0) {
...
//
//        if (returnType[0] == GDTypeEncodingObjcObject || returnType[0] == GDTypeEncodingObjcClass) {
...
//        } else if (returnType[0] != GDTypeEncodingVoid) {
//            NSAssert(methodSignature.methodReturnLength, @"Memory corruption lies ahead");
//
//            if (returnType[0] == GDTypeEncodingStructBegin) {
...
//                        *error = [NSError
//                            errorWithDomain:GDRuntimeUtilityErrorDomain
//                            code:GDRuntimeUtilityErrorCodeInvocationFailed
...
+ (NSValue *)valueForNumberWithObjCType:(const char *)typeEncoding fromInputString:(NSString *)inputString {
    ...
    if (strlen(typeEncoding) > 1) {
        ...
        if ([type isEqualToString:@GDEncodeClass(NSDecimalNumber)]) {
            return [NSDecimalNumber decimalNumberWithString:inputString];
        } else if ([type isEqualToString:@GDEncodeClass(NSNumber)]) {
            return number;
        }
    ...
    GDTypeEncoding type = typeEncoding[0];
    uint8_t value[32];
...
//    if (structEncoding && structEncoding[0] == GDTypeEncodingStructBegin) {
...
//            if (GDGetSizeAndAlignment(structEncoding, &structSize, &fieldAlignment)) {
//                NSUInteger runningFieldIndex = 0;
//                NSUInteger runningFieldOffset = 0;
//                const char *typeStart = equals + 1;
//
//                while (*typeStart != GDTypeEncodingStructEnd) {
//                    NSUInteger fieldSize = 0;
//                    // If the struct type encoding was successfully handled by
//                    // GDGetSizeAndAlignment above, we *should* be ok with the field here.
...
+ (NSString *)appendName:(NSString *)name toType:(NSString *)type {
    ...
    if ([type characterAtIndex:type.length - 1] == GDTypeEncodingCString) {
...
+ (NSString *)readableTypeForEncoding:(NSString *)encodingString {
    ...
    const NSUInteger fieldNameOffset = [GDRuntimeUtility fieldNameOffsetForTypeEncoding:encodingCString];
    if (fieldNameOffset > 0) {
        ...
        NSArray<NSString *> *const fieldNames = [fieldNamesString
            componentsSeparatedByString:[NSString stringWithFormat:@"%c", GDTypeEncodingQuote]
        ];
    ...
    // Objects
    if (encodingCString[0] == GDTypeEncodingObjcObject) {
        NSString *class = [encodingString substringFromIndex:1];
        class = [class stringByReplacingOccurrencesOfString:@"\"" withString:@""];
        if (class.length == 0 || (class.length == 1 && [class characterAtIndex:0] == GDTypeEncodingUnknown)) {
            class = @"id";
        } else {
...
#undef TRANSLATE
    ...
    if (encodingCString[0] == GDTypeEncodingStructBegin) {
        ...
        if (equals) {
            ...
            if (nameStart[0] == GDTypeEncodingUnknown) {
...
//    const NSUInteger fieldNameOffset = [GDRuntimeUtility fieldNameOffsetForTypeEncoding:type];
//    if (fieldNameOffset > 0) {
...
//
//    NSValue *value = nil;
//    if (GDGetSizeAndAlignment(type, nil, nil)) {
//        @try {
