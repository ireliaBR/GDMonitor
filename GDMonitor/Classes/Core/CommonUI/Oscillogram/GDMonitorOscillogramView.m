...
@implementation GDMonitorPoint

@end

@interface GDMonitorOscillogramView()<UIScrollViewDelegate>
...
@implementation GDMonitorOscillogramView

- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        _kStartX = kGDMonitorSizeFrom750_Landscape(52);
        ...
        _bottomLine = [[UIView alloc] initWithFrame:CGRectMake(_kStartX, self.frame.size.height - kGDMonitorSizeFrom750_Landscape(1), self.frame.size.width, kGDMonitorSizeFrom750_Landscape(1))];
        _bottomLine.backgroundColor = UIColorFromRGB(0x999999);
        [self addSubview:_bottomLine];
        
        _lowValueLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.frame.size.height-kGDMonitorSizeFrom750_Landscape(28)/2, _kStartX, kGDMonitorSizeFrom750_Landscape(28))];
        ...
        _highValueLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, -kGDMonitorSizeFrom750_Landscape(28)/2, _kStartX, kGDMonitorSizeFrom750_Landscape(28))];
        ...
        _tipLabel.textColor = UIColorFromRGB(0x00DFDD);
        _tipLabel.textAlignment = NSTextAlignmentCenter;
        _tipLabel.font = [UIFont systemFontOfSize:kGDMonitorSizeFrom750_Landscape(20)];
...
- (void)addHeightValue:(CGFloat)showHeight andTipValue:(NSString *)tipValue{
    ...
    GDMonitorPoint *point = [[GDMonitorPoint alloc] init];
    point.x = _x;
    point.y = _y;
    ...
    if (_pointList.count > _numberOfPoints) {
        ...
        for (GDMonitorPoint *point in _pointList) {
            point.x -= step;
            if (point.x < _kStartX) {
...
- (void)drawLine{
    ...
    GDMonitorPoint *point = self.pointList[0];
    ...
    layer.strokeColor = UIColorFromRGB(0x00DFDD).CGColor;
...
- (void)addPointLayer:(CGPoint)point{
    ...
    pointLayer.backgroundColor = UIColorFromRGB(0x00DFDD).CGColor;
    pointLayer.cornerRadius = 2;
    pointLayer.frame = CGRectMake(point.x-kGDMonitorSizeFrom750_Landscape(8)/2, point.y-kGDMonitorSizeFrom750_Landscape(8)/2, kGDMonitorSizeFrom750_Landscape(8), kGDMonitorSizeFrom750_Landscape(8));
    [_pointLayerList addObject:pointLayer];
}

- (void)drawTipViewWithValue:(NSString *)tip point:(GDMonitorPoint *)point time:(NSString *)time {
    if (_tipLabel.hidden) {
        _tipLabel.hidden = NO;
    }
    ...
    [_tipLabel sizeToFit];
    _tipLabel.frame = CGRectMake(point.x, self.frame.size.height-point.y-_tipLabel.frame.size.height, _tipLabel.frame.size.width, _tipLabel.frame.size.height);
    //self.indicatorLayer.frame = CGRectMake(point.x, 0, 1, self.bottomLine.GDMonitor_bottom);
}
