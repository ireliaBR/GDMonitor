...
@implementation UIImage (Monitor)

+ (nullable UIImage *)monitor_imageNamed:(NSString *)name{
    if(name &&
       ![name isEqualToString:@""]){
        NSBundle *bundle = [NSBundle bundleForClass:NSClassFromString(@"GDMonitorManager")];
        NSURL *url = [bundle URLForResource:@"GDMonitor" withExtension:@"bundle"];
...
+ (UIImage *)monitor_imageWithColor:(UIColor *)color {
    return [self monitor_imageWithColor:color size:CGSizeMake(1, 1)];
}
